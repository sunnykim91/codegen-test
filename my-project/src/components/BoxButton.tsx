import React, { memo, ReactNode, ButtonHTMLAttributes, isValidElement, cloneElement } from "react";
...
const getVariantStyle = (color: BoxButtonColor, variants: BoxButtonVariants, tinted: boolean) => {
  if (variants === "out-line") {
    if (color === "primary") {
      return {
        ...
        border: `1px solid ${tinted ? "var(--stroke-primary-default)" : "var(--stroke-primary-strong-3)"}`,
    ...
    return {
      background: "transparent",
      color: tinted ? "var(--text-icon-gray-subtle-2)" : "var(--text-icon-gray-default)",
      border: `1px solid ${tinted ? "var(--stroke-gray-strong)" : "var(--stroke-gray-strong-2)"}`,
      pressedOverlay: "var(--state-pressed-black)",
    };
  }
  ...
  return {
    ...
    border: "none",
    pressedOverlay: color === "gray" && !tinted ? "var(--state-pressed-white)" : "var(--state-pressed-black)",
  };
};
...
const BoxButtonComponent = ({
  ...
  return (
    <button className={className} style={buttonStyle} onClick={handleClick} disabled={isDisabled} {...props}>
